...
def mlp3():
    ...
    return resultados, mejor
    arquitectura = Dense(48, sigmoid) + Dense(10, softmax)
    num_repeticiones = 5
...
def mlp7():
    ...
    dropout_rate = 0.15       # % de neuronas a apagar 
    l2_lambda = 0.000001          
...
def cnn2():
    ...
    kernel_sizes = [(1,1),(2,2),(3, 3),(3, 5),(5,3),(5,5),(7,7),(9,9),(11,11)]
    resultados = []
    ...
    for kernel_size in kernel_sizes:
        ...
        for rep in range(5):
            model = keras.Sequential([
                ...
                # Bloque 1
                layers.Conv2D(16, kernel_size, activation='relu',kernel_initializer='he_normal', padding='same'),
                layers.MaxPooling2D((2, 2)),
                
                # Bloque 2 
                layers.Conv2D(32, kernel_size, activation='relu',kernel_initializer='he_normal', padding='same'),
                layers.MaxPooling2D((2, 2)),
    ...
    mejor = max(resultados, key=lambda x: x['test_acc'])
    plot_comparacion(resultados, 'kernel_size', 'Tamaño de Kernel', 'cnn2_comparacion_kernel.png')
    plot_history(mejor['history'])    
    return resultados, mejor
...
def cnn3():
    ...
    kernel_size = (3, 3)              # mejor del cnn2
    ...
    for rep in range(num_repeticiones):
        ...
        for filters in conv_filters:
            model.add(layers.Conv2D(filters, kernel_size, activation='relu',kernel_initializer='he_normal', padding='same'))
    ...
    cnn1_acc = 0.6885 #Baseline
    cnn1_loss = 0.8912
    cnn1_epochs = 21.0
    cnn1_tiempo = 16.7

    mejora = (avg_test_acc - cnn1_acc) * 100
    ...
    plot_history(avg_history)
    plot_comparacion(comparacion, 'modelo', 'Modelo', 'cnn3_comparacion_baseline.png')
    Y_pred = model.predict(X_test, verbose=0)
    plot_confusion_matrix(Y_test, Y_pred)
